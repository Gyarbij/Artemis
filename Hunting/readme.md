...
![KQL visualized](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/kql-pipe.png?raw=true)
...
| where UserPrincipalName == "gyarbij@example.com"
| where ResultType == "0"
...
SigninLogs
| where TimeGenerated > ago(14d)
| where UserPrincipalName == "gyarbij@testdomain.com"
```

Next we ask Sentinel to only find logs where UserPrincipalName is equal to "gyarbij@testdomain.com"
...
This will search our SigninLogs table, over the last 14 days, for exact matches where our UserPrincipalName equals gyarbij@testdomain.com. In KQL == is case sensitive, so if you search for gyarbij@TESTdomain.com and the username is actually gyarbij@testdomain.com, you won't get any results. The non case sensitive equivalent is =~
...
This will find any matches for gyarbij@testdomain.com regardless of case sensitivity.
...
SigninLogs
| where TimeGenerated > ago(14d)
| where UserPrincipalName contains "gyarbij"
```

This will find any log entries where the UserPrincipalName contains gyarbij, if you had gyarbij@testdomain.com and gyarbij@anotherdomain.com data, it would find both. The contains operator is not case sensitive, but you can use contains_cs to make it case sensitive.
...
SigninLogs
| where TimeGenerated > ago(14d)
| where UserPrincipalName startswith "gyarbij"
...
SigninLogs
| where TimeGenerated > ago(14d)
| where * contains "gyarbij"
```

This will search the SigninLogs table for any field that contains gyarbij.
...
This query would find all SigninLogs where the UserPrincipalName does not equal gyarbij@testdomain.com
...
This query would find all SigninLogs where the UserPrincipalName does not contain gyarbij
...
This query searches for SigninLogs data from the last 14 days, where the UserPrincipalname equals gyarbij@testdomain.com, where the ResultType is 0, where the application display name equals "Microsoft Teams", then for each match on that query it returns the TimeGenerated, Location, IPAddress and the UserAgent.
...
This query will look up the SigninLogs table for any events in the last 14 days, for any matches for gyarbij@testdomain.com, where the result is a success (ResultType == 0) and then summarize those events by the application display name.
...
This will return a single record for each distinct application gyarbij@testdomain.com signed into.
...
This query looks for all signin logs over the last 14 days, that have gyarbij@testdomain.com as the UserPrincipalname, that are successful and then returns the latest record.
...
This will output a list of applications that gyarbij@testdomain.com has signed into to a list called AppList.
...
This will make a list of applications that gyarbij@testdomain.com has signed into separated into one list per day.
...
This query summarizes all the applications gyarbij@testdomain.com has signed into in the last 14 days, then renders the output as a piechart.

![KQL Piechart](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-piechart.png?raw=true)
...
![KQL Column Chart](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-columnchart.png?raw=true)
...
![KQL Bar Chart](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-barchart.png?raw=true)
...
This visualizes all signins by gyarbij@testdomain.com per day over the last 14 days and displays it as a timechart.

![KQL Timechart](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-timechart.png?raw=true)
...
![KQL Areachart](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-areachart.png?raw=true)
...
![KQL Time Column Chart](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-timecolumnchart.png?raw=true)
...
![KQL Time Bar Chart](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-timebarchart.png?raw=true)
...
![KQL Time Column Chart Stacked](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-timecolumnchartstacked.png?raw=true)
...
![KQL Time Column Chart Unstacked](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-timecolumnchartunstacked.png?raw=true)
...
![KQL Time Column Chart Unstacked Renamed](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-timecolumnchartnames.png?raw=true)
...
![KQL Time Column Chart Outlook, OneDrive, SharePoint](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/render-timecolumn-outlookonedrivesharepoint.png?raw=true)
...
[
'Name=gyarbij,UPNSuffix=testdomain.com,AadTenantId=345c1234-a833-43e4-1d34-123440a5bcdd1,AadUserId=cf6f2df6-b754-48dc-b7bc-c8339caf211,DisplayName=Test User,Type=account'
];
```

This data is just a single string that looks like this.

![Parse](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/parse.png?raw=true)
...
![Parse 1](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/parse1.png?raw=true)
...
![Parse 2](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/parse2.png?raw=true)
...
![Parse 3](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/parse3.png?raw=true)
...
[
'Name=gyarbij,UPNSuffix=testdomain.com,AadTenantId=345c1234-a833-43e4-1d34-123440a5bcdd1,AadUserId=cf6f2df6-b754-48dc-b7bc-c8339caf211,DisplayName=Test User,Type=account',
'Display=gyarbij,UPN=testdomain.com,AadDirectoryId=345c1234-a833-43e4-1d34-123440a5bcdd1,AadObjectId=cf6f2df6-b754-48dc-b7bc-c8339caf211,Name=Test User,AccountType=account'
]
;
ExampleText
| parse TestData with * 'Name=' DisplayName ',UPNSuffix=' DomainSuffix ',AadTenantId=' AzureADTenantId ',' *
...
![Parse 4](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/parse4.png?raw=true)
...
![Parse 5](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/parse5.png?raw=true)
...
![Split 1](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/split1.png?raw=true)
...
![Split 2](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/split2.png?raw=true)
...
[
'Name=gyarbij,UPNSuffix=testdomain.com,AadTenantId=345c1234-a833-43e4-1d34-123440a5bcdd1,AadUserId=cf6f2df6-b754-48dc-b7bc-c8339caf211,DisplayName=Test User,Type=account',
'Name=Gyarbij69,UPNSuffix=testdomain.com,AadTenantId=331c1234-a841-43e5-1d31-12220a5bcee1,AadUserId=cf6f2df6-b754-48dc-b7bc-c8339caf211,DisplayName=Test User 2,Type=account'
]
;
ExampleText
| extend Name = split(TestData,',')[0]
...
| extend AccountType = split(TestData,',')[5]
| project Name, DomainSuffix, AzureADTenantId, AzureADUserId, DisplayName, AccountType
| where Name contains "gyarbij"
```

We get only one hit, where Name contains "gyarbij", our second record where Name contains "Gyarbij69" isn't found.

![Split 3](https://github.com/gyarbij/KQLQueries/blob/main/Diagrams/split3.png?raw=true)
